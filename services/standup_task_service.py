...
class StandupTaskService():
    ...
    def create_standup_task_with_work(self, standup_task_form, standup_task_work_form):
        if not standup_task_form.is_valid() or not standup_task_form.is_valid():
            errors = standup_task_form.errors + standup_task_work_form.errors
            raise ValidationError(errors)

        standup_task = self.standup_task_repository.create_standup_task(standup_task_form)
        standup_task_work = self.standup_task_repository.create_standup_task(standup_task_work_form)
